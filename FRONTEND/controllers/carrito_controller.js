...
async function showProducts() {
    ...
    for(let i = 0; i < user_carrito.length; i++) {
        ...
        prod_div.appendChild(min);
        prod_div.appendChild(text_cant);
        prod_div.appendChild(cant_span);
        prod_div.appendChild(max);
...
async function html_stripe() {
    // Clave publica dada por Stripe
    const stripe = Stripe('pk_test_51RMLSWR14tdVGj3jmSYDhpJobxGC0m92CdyTD5PLVQb29SZjdIQOt9j94xieGP5C2FmCQHL8FP316OaOFIkFRN0l00snMdpsl5');
    ...
    // Parte de Total
    const total_price = cont_original_price - cont_desc;
    // Boton de pagar dentro del html
    const pay_button = document.getElementById("pay_button");
    // Cuando se le haga click al boton...
    pay_button.addEventListener('click', async(event) => {
        event.preventDefault();

        try {
            const total = total_price;
            const items = cant_span;

            // Se crea la sesion de pago dentro de la bd
